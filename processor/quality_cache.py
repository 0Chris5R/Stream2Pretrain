        ...
        with self._lock:
            for key, text in zip(keys, texts, strict=True):
                ...
                if row is None:
                    ...
                    if "score" not in payload and "edu_score" in payload:
                        payload["score"] = payload.pop("edu_score")
                    for diagnostic in (payload.get("diagnostic_scores") or {}).values():
                        if "score" not in diagnostic and "edu_score" in diagnostic:
                            diagnostic["score"] = diagnostic.pop("edu_score")
