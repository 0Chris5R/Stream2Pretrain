...
def accepted_trajectories(
    content: bytes, *, trajectory_id: str | None = None
) -> list[dict[str, Any]]:
    """Read accepted SFT trajectories, optionally selecting one artifact."""
    with tarfile.open(fileobj=io.BytesIO(content), mode="r:gz") as archive:
        try:
            member = archive.getmember(f"{_ROOT}trajectories/accepted.jsonl")
        except KeyError:
            return []
        handle = archive.extractfile(member)
        if handle is None:
