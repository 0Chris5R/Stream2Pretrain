    ...
    def preserve(self, doc_id: str, capsule: bytes | None, uri: str | None) -> str | None:
        if capsule is not None:
            ...
            try:
                ...
                try:
                    stored_payload = body.read()
                finally:
                    body.close()
                payload = (
                    gzip.decompress(stored_payload)
                    if stored_payload.startswith(b"\x1f\x8b")
                    else stored_payload
                )
        ...
        if not stored_payload.startswith(b"\x1f\x8b"):
            stored_payload = gzip.compress(payload, mtime=0)
        key = f"scientific-evidence/{doc_id.removeprefix('sha256:')}/{digest}.json.gz"
        self.s3.put_object(
            Bucket=self.bucket,
            Key=key,
            Body=stored_payload,
