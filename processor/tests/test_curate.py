...
def test_segment_models_batch_the_sole_quality_classifier(
    ...
    for segment in segments:
        ...
        assert result.quality.score == float(len(segment.text))
...
def test_document_micro_batch_scores_all_eligible_segments(
    cfg: ProcessorConfig,
    long_english_text: str,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    classifier = _BatchQualityScorer()
    state.source_quality = classifier
    ...
    try:
        ...
        assert all(outcome.error is None for outcome in outcomes)
        assert sorted(len(batch) for batch in classifier.batches) == [2] * 6
        assert sorted(text for batch in classifier.batches for text in batch) == sorted(
            [
...
def test_hf_card_deterministic_reject_skips_classifier_inference(
    cfg: ProcessorConfig,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("S2P_CURATOR_CLASSIFIER_BATCH_SIZE", "2")
    ...
    try:
        ...
        assert gold.segment_scores[0].source_quality_score is None
        assert gold.classifier_revision == "not-run:deterministic-reject"
    finally:
        state.close()


def test_known_near_duplicate_skips_classifier_before_durable_rejection(
    cfg: ProcessorConfig,
    long_english_text: str,
) -> None:
    state = build_state(cfg)
