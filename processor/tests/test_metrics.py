...
def test_processor_metrics_render_dashboard_contract() -> None:
    ...
    metrics.record_curated(source_feed="arxiv-rss", quality_score=3.25)
    metrics.record_dropped(reasons=["license_excluded"], quality_score=0.75)
    metrics.record_decon_scan(benchmarks=["MMLU"])
    metrics.record_iceberg_flush(rows=2, seconds=0.12)

    body = metrics.render_prometheus().decode("utf-8")

    assert 's2p_processor_ingested_total{namespace="stream2pretrain",source="arxiv-rss"} 1.0' in body
    assert 's2p_processor_curated_total{namespace="stream2pretrain",source="arxiv-rss"} 1.0' in body
    assert 's2p_processor_dropped_total{namespace="stream2pretrain",reason="license_excluded"} 1.0' in body
