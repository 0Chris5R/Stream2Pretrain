    ...
    return {
        ...
        "quality": {
            "source-pretrain-quality": {
                ...
                "revision": "source-quality@pinned",
            },
        },
        "kenlm": {
...
def test_remote_model_facades_preserve_revisions_and_results() -> None:
                    ...
                    "results": [
                        {
                            "score": 4.25,
                            "revision": "source-quality@pinned",
                            "diagnostic_scores": {"arxiv-math-reasoning": {"score": 3.5}},
                        }
    ...
    classifier = RemoteQualityClassifier(client, "source-pretrain-quality")
    kenlm = RemoteKenLMScorer(client)

    assert classifier.score("paper").score == 4.25
    assert classifier.score("paper").diagnostic_scores["arxiv-math-reasoning"]["score"] == 3.5
    assert [result.revision for result in classifier.score_many(["paper", "paper"])] == [
        "source-quality@pinned",
        "source-quality@pinned",
    ]
    assert classifier.revision == "source-quality@pinned"
...
def test_quality_batch_requires_one_result_per_input() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200,
            json={"results": [{"score": 4.0, "revision": "source-quality@pinned"}]},
        )
...
def test_oversize_combined_batch_preserves_the_singleton_path(monkeypatch) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        return httpx.Response(
            200,
            json={"results": [{"score": 4.0, "revision": "source-quality@pinned"} for _ in texts]},
        )
...
def test_six_concurrent_calls_lease_six_distinct_ready_endpoints() -> None:
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            ...
            if request.url.path == "/v1/quality:batch":
                ...
                return httpx.Response(
                    ...
                    json={"results": [{"score": 4.25, "revision": "source-quality@pinned"}]},
                )
            return httpx.Response(404)
    ...
    assert [result.revision for result in results] == ["source-quality@pinned"] * 6
    ...
    def endpoint_factory(endpoint: str) -> httpx.Client:
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            ...
            return httpx.Response(
                200,
                headers={"X-S2P-Model-Backend": backend},
                json={"results": [{"score": 4.5, "revision": "source-quality@pinned"}]},
            )
...
def test_initial_partial_availability_uses_only_revision_matching_endpoint() -> None:
        ...
        def handler(request: httpx.Request) -> httpx.Response:
            if request.url.path == "/v1/metadata":
                ...
                if backend == "drifted":
                    ...
                    classifier = quality["source-pretrain-quality"]
                    assert isinstance(classifier, dict)
                    classifier["revision"] = "source-quality@wrong"
                return httpx.Response(200, json=metadata)
            inference_backends.append(backend)
            return httpx.Response(
                200,
                headers={"X-S2P-Model-Backend": backend},
                json={"results": [{"score": 4.75, "revision": "source-quality@pinned"}]},
            )
    ...
    assert result.revision == "source-quality@pinned"
