                ...
                "sections": [
                    {
                        "title": "Derivation",
                        "classifiers": {
                            "arxiv-posttrain-suitability": {"score": 5.0},
                            "arxiv-math-reasoning": {"score": 4.5},
                        },
                    }
                ],
            }
...
def test_composite_metric_is_not_overwritten_by_source_classifier(cfg, long_english_text):
    ...
    try:
        ...
        assert result.source_quality_score == 0.25
        assert result.quality_score != result.source_quality_score
...
def test_posttrain_http_family_reaches_second_stage_without_local_server(monkeypatch):
    ...
    handler.server = SimpleNamespace(
        runtime=SimpleNamespace(
            quality=object(),
            max_batch_items=2,
            quality_many=lambda family, texts: calls.append((family, texts)) or [{"score": 4}],
        )
    )
    monkeypatch.setattr(
        handler,
