...
def _gold(index: int, *, route: str = "pretrain") -> GoldRecord:
    return GoldRecord(
        ...
        classifier_revision="source-quality-v1",
...
def test_legacy_quality_column_is_removed_without_rebuilding_history(tmp_path) -> None:
    path = tmp_path / "legacy-quality.duckdb"
    kwargs = dict(
        database_path=str(path), brokers="unused", decisions_topic="d", admissions_topic="a"
    )
    index = ServingIndex(**kwargs)
    ...
    connection.execute("DROP VIEW serving_decisions")
    connection.execute("DROP INDEX IF EXISTS serving_decision_key")
    connection.execute("ALTER TABLE _serving_decision_records ADD COLUMN edu_score DOUBLE")
    connection.execute("UPDATE _serving_decision_records SET edu_score = 1.0")
    connection.close()
    ...
    columns = {
        str(row[1])
        for row in connection.execute("PRAGMA table_info('_serving_decision_records')").fetchall()
    }
    assert "edu_score" not in columns
    assert connection.execute("SELECT source_quality_score FROM serving_decisions").fetchone() == (
        4.0,
    )
    assert (
        identity
