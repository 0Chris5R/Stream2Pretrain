...
def test_gold_loads_migrates_persisted_quality_fields() -> None:
    rec = GoldRecord(
        doc_id="sha256:" + "e" * 64,
        text="retained scientific section",
        lang="en",
        tokens=3,
        quality_score=4.1,
        ...
        valid_from=datetime(2026, 6, 15, tzinfo=UTC),
        scoring_version="v0.1.0",
        classifier_revision="historic",
        policy_revision="git:test",
        trace_id="0" * 32,
    )
    value = rec.model_dump(mode="json", by_alias=True)
    value["edu_score"] = value.pop("source_quality_score")
    value["segment_scores"] = [
        {
            ...
            "word_count": 3,
            "edu_score": 4.4,
            "finepdfs_edu_score": None,
        }
    ]

    parsed = gold_loads(json.dumps(value).encode())

    assert parsed.source_quality_score == 4.2
    assert parsed.segment_scores[0].source_quality_score == 4.4


def test_upgrade_gold_record_migrates_bytewax_recovery_object() -> None:
    rec = GoldRecord(
        doc_id="sha256:" + "d" * 64,
        text="retained scientific section",
        lang="en",
    ...
    rec.__dict__["edu_score"] = rec.__dict__.pop("source_quality_score")

    parsed = upgrade_gold_record(rec)

    assert parsed.source_quality_score == 4.2
    assert parsed.projection_version == "document-v1"
