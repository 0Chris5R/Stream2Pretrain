...
def test_evidence_survives_missing_transient_object_without_truncation() -> None:
    ...
    assert ScientificDocument.model_validate_json(gzip.decompress(stored[0]["Body"])) == document
    assert stored[0]["ContentEncoding"] == "gzip"
...
def test_legacy_evidence_is_copied_exactly() -> None:
    ...
    assert gzip.decompress(stored[0]["Body"]) == payload
    assert stored[0]["ContentEncoding"] == "gzip"
