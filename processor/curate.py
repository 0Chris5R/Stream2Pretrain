...
def _weighted_quality(results: Sequence[QualityScore]) -> float:
    ...
    return sum(result.score * max(1, result.tokens) for result in results) / total if total else 0.0
...
def _source_quality_report(
    ...
    for task in sorted(tasks):
        ...
        best_row, best = max(head_rows, key=lambda pair: pair[1]["score"])
        ...
        diagnostic_heads[task] = {
            ...
            "aggregation": "maximum",
            "weighted_mean": sum(value["score"] * value["tokens"] for _, value in head_rows)
            / head_tokens,
            "mean": sum(value["score"] for _, value in head_rows) / len(head_rows),
...
def curate_one(state: CurateState, silver: SilverRecord) -> GoldRecord:
    ...
    for (segment, sanitization), safe_segment in zip(
        ...
        if quality_result is not None:
            source_quality_score = quality_result.score
    ...
    source_quality_score, perplexity, perplexity_bucket = aggregate_segment_scores(segment_scores)
    source_quality_score = float(quality_diagnostics["score"]) if quality_diagnostics else 0.0
    measured_buckets = [
    ...
    structure = source_scores(
        retained_view, quality_score=source_quality_score, quality_applicable=True
    )
    hf_assessment = (
        assess_hf_card(
