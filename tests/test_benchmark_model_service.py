...
def test_distribution_gate_runs_real_inference_and_checks_batch_parity(
        ...
        *,
            ...
            return (
                {
                    "ready": True,
                    "quality": {
                        "source-pretrain-quality": {"revision": "source-quality@test"},
                    },
                },
                "backend-a",
            )
        ...
        if url.endswith("/v1/quality:batch"):
            ...
            return (
                {
                    "results": [
                        {"score": len(str(text)), "revision": "source-quality@test"}
                        for text in texts
                    ]
                },
                backend,
        ...
        return (
            {"score": len(text), "revision": "source-quality@test"},
            backend,
        )
    ...
    assert result["revision"] == "source-quality@test"
...
def test_distribution_gate_scales_sample_and_fairness_with_backend_count(
    ...
    def fake_request(
        ...
        *,
        ...
        text = str(payload["text"])
        return ({"score": len(text), "revision": "source-quality@test"}, backend)
