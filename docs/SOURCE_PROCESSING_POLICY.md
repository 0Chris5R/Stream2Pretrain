...
| Hugging Face model card | Exact-revision README prose split by Markdown section after YAML, fenced-code, HTML, and asset removal. | Official model-card structure gate; FineWeb-Edu and FinePDFs Edu v2 same-section comparison; CPU language ID; redactable PII handling; exact/MinHash dedup; E5 decontamination. C4/Gopher and KenLM remain off. | Substantive technical documentation enters pretraining only. Template, upload, marketing, wrong-type, and insufficient cards quarantine. |
| Hugging Face dataset card | Exact-revision README prose split by Markdown section after YAML, fenced-code, HTML, and asset removal. Dataset rows and hosted binaries are excluded. | Official dataset-card structure gate plus the same learned-model comparison, privacy, dedup, and decontamination policy as model cards. | Substantive dataset documentation enters pretraining only. It never licenses or imports dataset rows. |
