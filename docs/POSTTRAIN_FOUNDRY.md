...
model review. Stream2Pretrain applies that pattern locally: the paper is the
source of truth, SFT and RL have different acceptance contracts, model judgments
are never sufficient for RL, and named human review is an additional quality
feature rather than a missing stage.

Primary references: [Kimi K3](https://arxiv.org/html/2607.24653),
[Kimi K2.5](https://arxiv.org/html/2602.02276),
[Kimi k1.5](https://github.com/MoonshotAI/Kimi-k1.5),
[Tulu 3](https://github.com/allenai/open-instruct/blob/main/docs/tulu3.md), and
...
Qwen3.8 critiques the merged graph under a dedicated critic prompt. Up to two
bounded repair passes may add, replace, or remove nodes and edges, each followed
by a fresh critic pass. References to nonexistent spans or objects, dangling edges, invalid
...
outcome. Code, not the model's route label, makes that routing decision. The
answerability model checks answerability and leakage and cannot downgrade an RL
task for being insufficiently novel. Valuable open-ended synthesis is proposed
directly as SFT. A public task never becomes hard merely by demanding more
schema fields.

If a task batch is structurally valid but every proposal fails deterministic
specification checks, one bounded repair receives the exact failures and the
original paper evidence. If that also fails, one final paper-grounded recovery
attempt requests one or two substantive SFT tasks instead of discarding the
paper because of an empty model response. If an answerability or grounding response omits an ID,
one bounded completion pass audits only the missing items while preserving all
existing decisions. A still-missing item fails closed. These repairs address
provider formatting and omission errors without lowering any scientific or
executable acceptance threshold.
...
enter an answerability, solver, grounding, or solution-repair prompt. Hidden
targets become releasable only when a blind solver independently derives a
matching answer, the grounding critic accepts its science, and the executable
contract passes. Its prompt says that the readable report is the scientific
...
For `scientific-reasoning-v2`, Qwen3.8 proposes an executable `VerifierSpec`, an
independent verifier critic audits it, and one bounded repair is available when
the critic identifies a release-blocking false-positive or false-negative risk.
Every proposal and repair is normalized against the reviewed hidden task, and
mandatory task-derived predicates are injected, so the model cannot remove or
alter the scientific contract. Invalid output falls back to the deterministic
task-derived verifier. Predicate types cover required nodes, evidence
...
dependency, or bind an equation through an explicit derivation relation. The
verifier checks submitted target-specific intermediate and
...
retain false-positive and false-negative counts, every check, mutation totals,
per-trajectory critic decisions, and details. SFT acceptance requires an
independently grounded answer plus positive and equivalent execution of its
answer-facing report, numeric, symbolic and configuration checks, metamorphic
consistency, deterministic replay, security, and no false negatives. Graph
ordering, relation serialization, adversarial and mutation results remain
diagnostics for SFT because they measure an RL reward rather than the quality of
...
rejected artifact in its original SFT or RL pool with the task, exact failure,
solver attempts and prompt-trace lineage. No routed RL failure disappears into
a paper-level rejection.
