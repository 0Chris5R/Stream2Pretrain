...
0.10*(structural/5) + 0.10*(source_quality/5)`, clamped to `[0,1]`.

Composite quality is a renormalized weighted mean of applicable signals:
source-specific learned quality 0.70, structure 0.20, and language confidence
0.10. Gopher, C4, and KenLM remain independent diagnostics. The result is
multiplied by five.
...
deterministic verifier or RL compiled verifier, executable suite, signed package,
and human audit. Schema-invalid graph patches, critiques, solver turns, and
verifier critiques receive one structure-only repair. A second invalid verifier
...
When no first-pass proposal survives deterministic task validation, the one
semantic repair user prompt is exactly:

```text
The first proposal contained no usable task after deterministic validation. Repair the existing supported ideas against the exact errors below. Do not relax the quality bar, invent evidence, or manufacture a finite RL answer. If an idea is scientifically valuable but not deterministically verifiable, route it to SFT. Do not return an empty list merely because no RL task is possible: a paper-specific derivation explanation, comparison, assumption analysis, or synthesis can still be valuable SFT. Return no more than {count} tasks, and return an empty list only when the paper truly supports none.
...
```text
The prior audit omitted task IDs. Return the complete AnswerabilityBatch. Preserve every existing decision verbatim and independently audit each missing task under the same strict criteria. Do not weaken a negative decision or invent an ID.
MISSING_TASK_IDS:
...
When a valid response omits one or more trajectory decisions, the completion
user prompt is exactly:

```text
The prior scientific grounding audit omitted trajectory IDs. Return the complete GroundingCritique. Preserve every existing decision verbatim and independently audit each missing trajectory under the same strict criteria. Do not infer correctness from another trajectory and do not invent an ID.
...
```text
SYSTEM:
Repair a verifier critique to the exact JSON schema. Preserve its scientific judgment and findings; change only invalid structure or field types.
...
If that response is still structurally invalid, the critic result is recorded
as unavailable and rejected. The executable contract falls back to the
deterministic task-derived verifier; no model-authored condition is trusted.
...
Both SFT and RL execute the same diagnostic suite:
...
- false positives and false negatives are counted separately.
...
SFT uses a task-derived answer verifier and stores each independently grounded
solver trajectory when report and finite outcome, equivalent, metamorphic,
replay, security, and false-negative checks pass. Graph ordering and relation
serialization, adversarial and mutation results remain audit diagnostics because
they test an RL reward contract rather than a correct supervised target. RL first
lets the model compile a verifier, normalizes it against hidden truth, uses
deterministic fallback if construction or critic
repair is invalid, and requires every diagnostic suite gate, every adversarial
case, every mutation kill, and zero false positives before packaging an
