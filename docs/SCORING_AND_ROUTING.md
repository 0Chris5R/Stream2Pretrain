...
Current scoring id: `S2P_SCORING_VERSION`, default `pretrain-content-v4`
...
figures + 0.10 * structure/5 + 0.10 * source_quality/5`
...
The base weights are source-specific learned quality 0.70,
source-appropriate structure 0.20, and language confidence 0.10. In profiles
where language is not applicable, its weight is removed and the remaining
weights are renormalized. Gopher, C4, and KenLM remain separate diagnostics and
do not affect this display score. The composite supports sorting and dashboards
and is never presented as an individual model output.
...
A trained topic tagger is separate future work, not one of the four scalar
classifiers. Content tags remain document-inspection metadata and are not
