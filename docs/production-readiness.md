...
| PRD-006 | done | Code ingest contract | Stop emitting raw `CodeFileRecord` payloads to `raw.fetched` unless the processor dispatches that schema. Use one canonical wire model. | Tarball output + processor fetcher unit tests |
...
| PRD-011 | done | KEDA | Separate GitHub release jobs from tarball code output and scale the worker from broker-visible job lag. | Dual-publish/filter tests plus rendered Kafka `ScaledObject` on `github.release.jobs` |
...
| PRD-013 | needs-measurement | Capacity | Measure Redpanda partitions, worker CPU/RAM, MinIO throughput, and seed-loader PVC sizing on the target k3s cluster. | `docs/capacity-benchmark.md`, `scripts/capacity_probe.py`, and a generated target-cluster report with commands and outputs |
