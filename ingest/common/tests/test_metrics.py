...
def test_ingest_metrics_render_feed_poll_contract() -> None:
    ...
    metrics.record_feed_poll(source_feed="github-events", outcome="success")
    metrics.record_feed_poll(source_feed="github-events", outcome="error")

    body = metrics.render_prometheus().decode("utf-8")

    assert "s2p_process_up 1" in body
    assert (
        's2p_feed_poll_total{namespace="stream2pretrain",source_feed="github-events",outcome="success"} 1'
        in body
    )
    assert (
        's2p_feed_poll_total{namespace="stream2pretrain",source_feed="github-events",outcome="error"} 1'
        in body
    )
